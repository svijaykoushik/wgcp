...
### 3.3. WebAssembly / Emscripten Storage Sync Protocol
WebAssembly games running under Emscripten require synchronization between the in-memory virtual directory (`MEMFS`) and browser database persistence. 
* **Internal FS Syncing**: The SDK does **not** expose a `syncFS()` wrapper or execute `FS.syncfs()` automatically. Because the Emscripten `FS` object is privately scoped inside the game’s private compiled script closure, forcing global namespace leakage on `window.FS` represents a security risk and causes namespace pollution.
* **Separation of Concerns**: The game engine is responsible for mounts, virtual directories, and executing its own native `FS.syncfs` loops internally. The custom template loader or game script then reads the written files (e.g. from IndexedDB or virtual filesystem via `FS.readFile()`) and calls the standard `WGCP.storage.save` and `load` APIs to persist state back to the portal wrapper.
