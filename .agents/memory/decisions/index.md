...
* [D-008 Adopt Configurable SDK Initialization (WGCPInitOptions) and Shift+Escape Fallback](D-008-configurable-sdk-init-and-escape-handling.md) - Record of the choice to introduce typed WGCPInitOptions in WGCP.init() with captureEscape: false support and Shift+Escape chord handling for native in-game pause menus. [Status: accepted]
* [D-009 Adopt Graceful Game Exit Handshake Protocol (WGCP_PREPARE_EXIT)](D-009-graceful-game-exit-handshake.md) - Decision to implement a coordinated portal-to-game teardown RPC handshake (WGCP_PREPARE_EXIT) with a 600ms safety budget and automated WASM storage flush to prevent save data loss during iframe unmounts. [Status: accepted]
